...
class WebhookService:
    ...
    async def process_shopify_webhook(
        ...
        try:
            ...
            logger.info(f"✅ Vente Shopify créée: {sale_id} - {total_price}€ - Commande #{order_number}")
    ...
    async def _get_attribution_from_code(self, short_code: str) -> Optional[Dict]:
        ...
        try:
            link = supabase.table('tracking_links').select('*').eq('short_code', short_code).execute()
            ...
            return {
                'influencer_id': link_data['influencer_id'],
    ...
    async def process_tiktok_webhook(
        ...
        try:
            ...
            total_amount = float(payment_info.get('total_amount', 0)) / 100  # TikTok envoie en centimes
            ...
            logger.info(f"✅ Vente TikTok Shop créée: {sale_id} - {total_amount}{currency} - Order #{order_id}")
    ...
    async def _verify_tiktok_signature(
        self,
        body: bytes,
        signature: str,
        merchant_id: str
    ...
    async def _get_influencer_by_tiktok_id(self, tiktok_creator_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            result = supabase.table('influencers').select('*').eq(
    ...
    async def _increment_link_conversion(self, link_id: str, revenue: float):
        ...
        try:
            link = supabase.table('tracking_links').select('conversions, revenue').eq('id', link_id).execute()
            ...
            if link.data:
                ...
                supabase.table('tracking_links').update({
                    ...
                    'revenue': current_revenue + revenue
                }).eq('id', link_id).execute()
        except Exception as e:
            logger.error(f"Erreur incrémentation conversion: {e}")
    
    async def _notify_influencer_sale(
        self,
        influencer_id: str,
        amount: float,
        commission: float
        ...
        try:
            ...
            influencer = supabase.table('influencers').select('user_id').eq('id', influencer_id).execute()
