...
def add_upload_endpoints(app, verify_token):
    ...
    async def upload_multiple_files(
        ...
        if len(files) > 10:
            raise HTTPException(
                status_code=400,
                detail="Vous ne pouvez uploader que 10 fichiers à la fois maximum"
            )
        ...
        try:
            ...
            for file in files:
                file_list.append({
                    ...
                    "url": supabase.storage.from_('uploads').get_public_url(f"{folder}/{file['name']}")
                })
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Erreur lors de la récupération de la liste: {str(e)}"
            )
