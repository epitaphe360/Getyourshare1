...
async def listen_to_database_changes():
    ...
    while True:
        try:
            ...
            response = supabase.table('commissions').select('*').gte('created_at', last_check.isoformat()).execute()
            ...
            response = supabase.table('payments').select('*').gte('updated_at', last_check.isoformat()).execute()
