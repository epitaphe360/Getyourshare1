...
class TrackingService:
    ...
    async def track_click(
        ...
        try:
            ...
            link_result = supabase.table('tracking_links').select('*').eq('short_code', short_code).execute()
    ...
    def _generate_attribution_cookie(
        self,
        link_id: str,
        influencer_id: str,
        click_id: str
    ...
    async def get_link_stats(self, link_id: str) -> Dict:
        ...
        try:
            ...
            clicks = supabase.table('click_logs').select('ip_address').eq('link_id', link_id).execute()
            ...
            # Taux de conversion
            conversion_rate = (len(sales.data) / link_data['clicks'] * 100) if link_data['clicks'] > 0 else 0
