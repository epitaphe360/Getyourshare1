...
class PaymentGatewayService:
    ...
    def create_payment(
        ...
        try:
            ...
            merchant_result = supabase.table('merchants')\
            ...
            transaction_insert = supabase.table('gateway_transactions')\
                .insert(transaction_data)\
                .execute()
            
            result['db_transaction_id'] = transaction_insert.data[0]['id'] if transaction_insert.data else None
    ...
    def process_webhook(
        ...
        try:
            ...
            signature_header = headers.get('X-Signature') or headers.get('X-CMI-Signature') or headers.get('kr-hash')
            signature_valid = gateway.verify_webhook(
                payload=raw_body or json.dumps(payload),
                signature=signature_header or '',
                secret=gateway_config.get(f'{gateway_type}_secret_key') or gateway_config.get(f'{gateway_type}_api_key', '')
            )
            
            if not signature_valid:
                logger.warning(f"Invalid webhook signature from {gateway_type} for merchant {merchant_id}")
                return {
                    'success': False,
            ...
            transaction_update = supabase.table('gateway_transactions')\
                .update({
                    'status': payment_info['status'],
                    'completed_at': datetime.now().isoformat() if payment_info['status'] == 'completed' else None,
    ...
    def get_transaction_status(self, transaction_id: str) -> Dict:
        ...
        try:
            result = supabase.table('gateway_transactions')\
                .select('*')\
...
class PayZenGateway:
    ...
    def create_payment(
        ...
        try:
            ...
            return {
                'success': False,
                'error': f"PayZen API error: {str(e)}",
                'gateway': 'payzen'
            }
        except Exception as e:
